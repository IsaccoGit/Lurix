            ...
            if (diff > 5000) {
                ...
                if (parseInt(msgs) === 5) {
                    let embed = new Discord.MessageEmbed()
                        ...
                        .setColor(`RED`)
                        .setDescription(`È stato rilevato uno **spam** da parte di ${message.author}, è stato messo in timeout per 30 minuti, lo slowmode di questo canale è stato impostato a 10 secondi`)
                    ...
                    message.member.timeout(1000 * 60 * 30, `Rilevazione di Spam`)
                    message.channel.setRateLimitPerUser(10)
                    message.channel.send({ embeds: [embed], components: [row] })
                    client.channels.cache.get(configId.channelsId.logs).send({ embeds: [embed2] })
                } else {
                    data.msgs = msgs;
                    checkspam.set(message.author.id, data)
                }
